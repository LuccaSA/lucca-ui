(function(){
	...
	.controller("luidUserPickerCtrl", ['$scope', '$httpBackend', '_', function($scope, $httpBackend, _) {
		...
		/****** NO FORMER EMPLOYEES *****/
		/********************************/
		$httpBackend.whenGET(/api\/v3\/users\/find\?formerEmployees=false\&.*/i).respond(200, RESPONSE_20_users_FE);
		$httpBackend.whenGET(/api\/v3\/users\/find\?clue=[b-z]{1}\&formerEmployees=false\&.*/i).respond(200, RESPONSE_10_users_FE);
		$httpBackend.whenGET(/api\/v3\/users\/find\?clue=[b-z]{2}\&formerEmployees=false\&.*/i).respond(200, RESPONSE_5_users_FE);
		$httpBackend.whenGET(/api\/v3\/users\/find\?clue=[b-z]{3}\&formerEmployees=false\&.*/i).respond(200, RESPONSE_2_users_FE);
		$httpBackend.whenGET(/api\/v3\/users\/find\?clue=[b-z]{4}\&formerEmployees=false\&.*/i).respond(200, RESPONSE_1_users_FE);
